...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content">
            ...
            <form role="form" method="POST" action="{{ route('cano.update') }}" id="dataForm">
                <div class="row">
                    <!-- left column -->
                                        ...
                                        <select class="form-control select2" name="steersman_id" id="steersman_id">
                                            @foreach ($data['steersman'] as $steerman)
                                                <option value="{{ $steerman->id }}"
                                                    {{ $steerman->id == old('steersman_id', $cano->steersman_id) ? 'selected' : '' }}>
                                                    {{ $steerman->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control" name="kind_of_property" id="kind_of_property">
                                            @foreach (Helper::getConstant('cano_kind_of_property') as $val => $label)
                                                <option value="{{ $val }}"
                                                    {{ $val == old('kind_of_property', $cano->kind_of_property) ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select class="form-control" name="type" id="type">
                                            @foreach (Helper::getConstant('cano_type') as $val => $label)
                                                <option value="{{ $val }}"
                                                    {{ $val == old('type', $cano->type) ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="col-md-12 box">
                                            <div class="box-body text-center">
                                                ...
                                                <div id="div-image" style="margin-top:10px">
                                                    @if ($cano->images->count() > 0)
                                                        @foreach ($cano->images as $k => $image)
                                                            <div class="col-md-3">
                                                                <img class="img-thumbnail"
                                                                    src="{{ Helper::showImage($image->image_url) }}"
                                                                    style="width:100%">
                                                                <div class="checkbox">
                                                                    <label><input type="radio" name="thumbnail_id"
                                                                            class="thumb" value="{{ $image->id }}"
                                                                            {{ $cano->thumbnail_id == $image->id ? 'checked' : '' }}>
                                                                        Ảnh đại diện </label>
                                                                    <button class="btn btn-danger btn-sm remove-image"
                                                                        type="button" data-value="{{ $image->image_url }}"
                                        ...
                                        <div class="col-md-12 box">
                                            <div class="box-body text-center">
                                                ...
                                                <div id="div-image1" style="margin-top:10px">
                                                    @if (!empty(old('certificate_of_registry_img', $cano->certificate_of_registry_img)))
                                                        @foreach (old('certificate_of_registry_img', $cano->certificate_of_registry_img) as $k => $image)
                                                            <div class="col-md-3">
                                                                <img class="img-thumbnail"
                                                                ...
                                                                <input type="hidden" name="certificate_of_registry_img[]"
                                                                    value="{{ $image }}">
                                                            </div>
                                                        @endforeach
                                                    @endif
                                        ...
                                        <div class="col-md-12 box">
                                            <div class="box-body text-center">
                                                ...
                                                <div id="div-image2" style="margin-top:10px">
                                                    <div id="div-image1" style="margin-top:10px">
                                                        @if (!empty(old('certificate_of_insurance_img', $cano->certificate_of_insurance_img)))
                                                            @foreach (old('certificate_of_insurance_img', $cano->certificate_of_insurance_img) as $k => $image)
                                                                <div class="col-md-3">
                                                                    <img class="img-thumbnail"
                                                                        src="{{ Helper::showImage($image) }}"
                                                                        style="width:100%">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="status" id="status">
                                        @foreach (Helper::getConstant('status') as $val => $label)
                                            <option value="{{ $val }}"
                                                {{ $val == (old('status', $cano->status) ?? 1) ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>

      ...
      <div class="col-md-12">
        ...
        <div class="box box-primary" id="resList">
          <div class="box-header with-border">
            <h3 class="box-title">Danh sách nhà hàng</h3>
          </div>
            <div class="box-body">
          ...
          <form class="form-inline" role="form" method="GET" action="{{ route('coupon-code.index') }}" id="searchForm">
            ...
            <div class="form-group">
              <select class="form-control select2" name="restaurant_id" id="restaurant_id">
                ...
                @foreach($restaurantList as $shop)
                <option value="{{ $shop->id }}" {{ $restaurant_id == $shop->id ? "selected" : "" }}>{{ $shop->name }}</option>
                @endforeach
              ...
              @if( $items->count() > 0 )
                ...
                <tr id="row-{{ $item->id }}">
                  ...
                  <td class="text-left">
                      ...
                      @if($arrSearch['status'] != 1)
                      <p class="tr-value text-bold restaurant" data-id="{{ $item->restaurant_id }}"><i class="fa fa-map-marker"></i> {{ $item->restaurant->name }}
                       </p>
                       @endif
                  </td>
                  @if($arrSearch['status'] == 1)
                  <td>
                    <p class="tr-value text-bold restaurant" data-id="{{ $item->restaurant_id }}">

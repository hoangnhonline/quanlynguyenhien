             ...
             @if( $items->count() > 0 )
              ...
              @foreach( $items as $item )
                ...
                <li style="border-bottom: 1px solid #ddd; padding-bottom: 10px; padding: 10px; font-size:17px; @if($item->driver_id == 0) background-color:#dee0e3 @endif">
                  ...
                  <a style="float:right; margin-left: 2px" onclick="return callDelete('{{ $item->title }}','{{ route( 'booking-xe-may.destroy', [ 'id' => $item->id ]) }}');" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>
                  @endif
                  @if($item->status != 3)
                  <a style="float:right; margin-left: 2px" class="btn btn-sm btn-success" href="{{ route('view-pdf', ['id' => $item->id])}}" target="_blank">PDF</a>
                   <a style="float:right; margin-left: 2px" href="{{ route( 'booking-xe-may.edit', $arrEdit ) }}" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></a>

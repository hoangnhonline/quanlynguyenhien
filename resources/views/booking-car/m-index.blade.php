             ...
             @if( $items->count() > 0 )
              ...
              @foreach( $items as $item )
                ...
                <li class="mb10" style="border-bottom: 1px solid #ddd; padding-bottom: 10px; padding: 10px; font-size:17px; @if($item->driver_id == 0) background-color:#dee0e3 @endif">
                  ...
                  @endif
                  <!---Nhà xe-->
                  @if($item->city_id == 2 && $item->partner_id > 0)
                    Nhà xe: <strong>{{ $nhaxeName[$item->partner_id] }}</strong>  <br>
                  @endif
                  <!---Tài xế-->
                  @if($item->driver_id > 0)
                    <p style="background-color: #ccc;text-align: center;padding:5px"><strong style="color: red">TX: {{ $item->driver->name }}</strong> <i class="glyphicon glyphicon-phone" style="font-size: 13px"></i> <a href="tel:{{ $item->driver->phone }}">{{ $item->driver->phone }}</a></p>
                  @else
                    @if($item->city_id == 1)

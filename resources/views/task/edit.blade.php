        ...
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="name" id="name"
                    value="{{ old('name', $detail->name) }}" placeholder="Tên công việc" autocomplete="off">
            </div>
            <div class="form-group">
                <label class="font-14 font-w-500">Công việc cha
                    <select class="form-control select2" name="parent_task_id" id="parent_task_id">
                        ...
                        @include('task.taskSelectList', [
                            'tasks' => $taskList,
                            'selectedValue' => old('parent_task_id', $detail->parent_task_id),
                            'prefix' => '',
                            'exclude_id' => null,
                        ])
                    </select>
                ...
                <select class="form-control select2" name="staff_id" id="staff_id">
                    ...
                    @foreach ($userList as $item)
                        <option data-department-id={{ $item->department_id }} value="{{ $item->id }}"
                            {{ old('staff_id', $detail->staff_id) == $item->id ? 'selected' : '' }}>
                            {{ $item->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select class="form-control select2" name="plan_id" id="plan_id">
                    ...
                    @foreach ($planList as $plan)
                        <option value="{{ $plan->id }}"
                            {{ old('plan_id', $detail->plan_id) == $plan->id ? 'selected' : '' }}>
                            {{ $plan->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-control select2" name="type" id="type">
                    <option value="1" {{ old('type', $detail->type) == 1 ? 'selected' : '' }}>Việc cố định
                    </option>
                    <option value="2"
                        {{ old('type', $detail->type) == 2 ? 'selected' : '' }}>
                        Việc phát sinh
                    </option>
                </select>
            </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select class="form-control select2" name="status" id="status">
                    @foreach (Helper::getConstant('task_status') as $value => $label)
                        <option value="{{ $value }}"
                            {{ old('status', $detail->status) == $value ? 'selected' : '' }}>
                            {{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select class="form-control select2" name="percent" id="percent">
                    ...
                    @for ($i = 5; $i <= 100; $i = $i + 5)
                        <option value="{{ $i }}"
                            {{ old('percent', $detail->percent) == $i ? 'selected' : '' }}>
                            {{ $i }}%</option>
                    @endfor
                </select>
            </div>
            <div class="form-group">
                ...
                <input type="text" class="form-control datetimepicker" name="from_date" id="from_date"
                    value="{{ old('from_date', !empty($detail->from_date) ? date('d/m/Y H:i', strtotime($detail->from_date)) : '') }}"
                    autocomplete="off">
            </div>
            <div class="form-group">
                ...
                <input type="text" class="form-control datetimepicker" name="to_date" id="to_date"
                    value="{{ old('to_date', !empty($detail->to_date) ? date('d/m/Y H:i', strtotime($detail->to_date)) : '') }}"
                    autocomplete="off">
            </div>
            <div class="form-group">
                ...
                <input type="text" class="form-control datetimepicker" name="completed_date" id="completed_date"
                    value="{{ old('completed_date', !empty($detail->completed_date) ? date('d/m/Y H:i', strtotime($detail->completed_date)) : '') }}"
                    autocomplete="off">
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
                ...
                <textarea name="description" class="form-control mb-3" id="description" rows="3" placeholder="Mô tả">{!! $detail->description !!}</textarea>
            </div>
        </div>
    </div>
</form>

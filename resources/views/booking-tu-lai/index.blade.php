...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="{{ route( 'booking-tu-lai.index') }}">
     Xe tự lái</a></li>
            ...
            @if( $items->count() > 0 )
              ...
              <tr id="row-{{ $item->id }}" @if($item->driver_id == 0) style="background-color:#dee0e3" @endif>
                ...
                <td>
                  ...
                  @endif </strong>
                  <br>
                   <a style="font-size:17px" href="{{ route( 'booking-tu-lai.edit', $arrEdit) }}">{{ $item->name }}</a> - <a href="tel:{{ $item->phone }}">{{ $item->phone }}</a></a>
                  <br>
                  @if(Auth::user()->role == 1 && !Auth::user()->view_only  || Auth::user()->id == 23)
                    Sales:
                    @if($item->user)
                ...
                <a href="{{ route( 'booking-payment.index', ['booking_id' => $item->id] ) }}&back_url={{ urlencode(Request::fullUrl()) }}" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-usd"></span></a>
                  @php $arrEdit = array_merge(['id' => $item->id], $arrSearch) @endphp
                  <a href="{{ route( 'booking-tu-lai.edit', $arrEdit ) }}&back_url={{ urlencode(Request::fullUrl()) }}" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></a>
                  @if((Auth::user()->role == 1 && !Auth::user()->view_only || Auth::user()->id == 23) && $item->status == 1)
                  <a onclick="return callDelete('PTX{{ $item->id }} - {{ $item->name }}','{{ route( 'booking-tu-lai.destroy', [ 'id' => $item->id ]) }}');" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>
                  @endif
                  @if((Auth::user()->role == 1 && !Auth::user()->view_only || Auth::user()->id == 23) && $item->status == 1)

      ...
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          <form class="form-inline" role="form" method="GET" action="{{ route('report.detail-by-type') }}" id="searchForm">
           ...
           <button type="button" class="btn btn-success btn-sm" id="export" style="margin-top: -5px">Export</button>
            ...
            <table class="table table-bordered table-hover" id="datatable">
                  ...
                  <tr >
                    ...
                    <td class="bg-primary text-right">Tổng
                      ...
                      <br>
                      {{ number_format($tong_so_phong) }} NL
                    ...
                    <td class="text-right bg-primary" style="vertical-align: middle;">{{ date('d', strtotime($date)) }}
                      ...
                      <br>
                      <span style="white-space: nowrap;">{{ isset($arrBk[$date]['tong_so_phong']) ? number_format($arrBk[$date]['tong_so_phong']) : "0" }} NL   </span>
                    </td>
                    @endforeach
                  </tr>
                  ...
                  <tr {{ $bg }}  data-level="{{ $level }}">
                    <td  rowspan="10"><p style="width: 100px !important; font-weight: bold;">{{ Helper::getLevel($level) }}</p></td>
                  ...
                  <tr {{ $bg }}>
                    <td style="font-weight: bold;">Lợi nhuận</td>
                    <td class="text-right">{{ number_format($arrByLevel[$level]['hoa_hong_cty']) }}</td>
                    @foreach($arrDate as $date)
                    <td class="text-right">
                      @if(isset($arrByLevel[$level][$date]) )
                      {{ number_format($arrByLevel[$level][$date]['hoa_hong_cty']) }}
                       @else
                      -
                      @endif
                      @if(isset($arrByLevel[$level][$date]) && $arrBk[$date]['tong_hh_cty'] > 0)
                      <br>({{ round($arrByLevel[$level][$date]['hoa_hong_cty']*100/$arrBk[$date]['tong_hh_cty']) }}%)
                      @endif
                  ...
                  <tr {{ $bg }}>
                    <td style="font-weight: bold;">Số phòng</td>
                    <td class="text-right">{{ number_format($arrByLevel[$level]['so_phong']) }}</td>
                    @foreach($arrDate as $date)

                    <td class="text-right">{{ isset($arrByLevel[$level][$date]) ? number_format($arrByLevel[$level][$date]['so_phong']) : '-' }}</td>
                    @endforeach
                  </tr>
                  <tr {{ $bg }}>
                    <td style="font-weight: bold;">Số đêm</td>
                    <td class="text-right">{{ number_format($arrByLevel[$level]['so_dem']) }}</td>
                    @foreach($arrDate as $date)
                    <td class="text-right">{{ isset($arrByLevel[$level][$date]) && $arrByLevel[$level][$date]['so_dem'] > 0 ? number_format($arrByLevel[$level][$date]['so_dem']) : '-' }}</td>
                    @endforeach
                  </tr>
                  <?php
                  //dd($arrByLevel);
                  ...
                  @for($k = 5; $k > 0; $k--)
                  <tr {{ $bg }}>
                    <td style="font-weight: bold;">{{ $k }} sao</td>
                    <td class="text-right">{{  isset($arrByLevel[$level][$k]) ? number_format($arrByLevel[$level][$k]['so_booking']) : "-" }}</td>
                    @foreach($arrDate as $date)
                    <td class="text-right">{{ isset($arrByLevel[$level][$date][$k]) && $arrByLevel[$level][$date][$k]['so_booking'] > 0 ? number_format($arrByLevel[$level][$date][$k]['so_booking']) : '-' }}</td>
                    @endforeach
                  </tr>
                  @endfor

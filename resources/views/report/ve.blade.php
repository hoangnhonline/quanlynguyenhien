          ...
          <div class="table-responsive" >
            ...
            <table class="table table-bordered table-hover">
                  ...
                  <tr {{ $bg }}  data-level="{{ $level }}">
                    <td  rowspan="4"><p style="width: 100px !important; font-weight: bold;">{{ Helper::getLevel($level) }}</p></td>
                  ...
                  <tr {{ $bg }}>
                    ...
                    <td class="text-right">{{ number_format($arrByLevel[$level]['so_ve']) }}</td>
                    @foreach($arrDate as $date)
                    <td class="text-right">{{ isset($arrByLevel[$level][$date]) ? number_format($arrByLevel[$level][$date]['so_ve']) : '-' }}</td>
                    @endforeach
                  </tr>
                  <tr {{ $bg }}>
                  ...
                  ?>
                  <!--
                  @foreach($arrTicketTypeId as $ticketTypeId)
                  <tr {{ $bg }}>
                    <td style="font-weight: bold;">{{ $ticketTypeArr[$ticketTypeId] }}</td>
                    <td class="text-right">{{  isset($arrByLevel[$level][$ticketTypeId]) ? number_format($arrByLevel[$level][$ticketTypeId]['so_ve']) : "-" }}</td>
                    @foreach($arrDate as $date)
                    <td class="text-right">{{ isset($arrByLevel[$level][$date][$ticketTypeId]) && $arrByLevel[$level][$date][$ticketTypeId]['so_ve'] > 0 ? number_format($arrByLevel[$level][$date][$ticketTypeId]['so_ve']) : '-' }}</td>
                    @endforeach
                  </tr>
                  @endforeach
                -->

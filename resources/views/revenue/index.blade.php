        ...
        <div class="panel-body">
          <form class="form-inline" role="form" method="GET" action="{{ route('revenue.index') }}" id="searchForm">
            <div class="form-group">
              <input type="text" class="form-control" autocomplete="off" name="id" placeholder="ID" value="{{ $arrSearch['id'] }}" style="width: 100px">
            </div>
            <div class="form-group">
            ...
            <div class="form-group" style="border-right: 1px solid #9ba39d">
              &nbsp;&nbsp;&nbsp;<input type="checkbox" name="not_kpi" id="not_kpi" {{ $not_kpi == 1 ? "checked" : "" }} value="1" class="search-form-change">
              <label for="not_kpi" style="color: red">KHÔNG TÍNH KPI&nbsp;&nbsp;&nbsp;&nbsp;</label>
            </div>
            <div class="form-group">
              <input type="text" class="form-control" name="content" value="{{ $content }}" placeholder="Nội dung">
            </div>
            ...
            <table class="table table-bordered table-hover" id="table-list-data">
              ...
              @if( $items->count() > 0 )
                ...
                <tr id="row-{{ $item->id }}">
                  ...
                  <td class="text-center"><strong style="color: red">{{ $item->id }}</strong><br></td>
                  <td width="150">
                    {{ date('d/m/Y', strtotime($item->pay_date)) }}
                  @if($item->status == 1)
                        <label class="label label-success label-sm">Đã thu</label>
                    @else
                        <label class="label label-danger label-sm">Chưa thu</label>
                    @endif
                  </td>
